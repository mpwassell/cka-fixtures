...
function LoadScoreCard(FixtureCode)
{
 try
 {
  ...
  //Construct the URL using the FixtureCode passed in
  var sURL = '../' + UploadDirectory + '/' + FixtureCode + '.htm';
    
  //Check if browser supports the showModalDialog function
  ...
  {
    //Open up a window
    var sDimensions = "height=650px, width=860px";
    var sOptions    = "status=no, resizable=yes, toolbar=no, menubar=no, location=no, titlebar=no, scrollbars=yes";
    var sFeatures   = sDimensions + " " + sOptions;
...
function LoadPlayerInfo(PlayerID)
{
 try
 {
  if (PlayerID == "")
  {
    //No player id specified so leave now
    return;
  }
    
  //Setup the parameters to window.showModalDialog()
  var sURL = "../PlayerInfo.htm";
  //var sDimensions = "dialogHeight:290px; dialogWidth:340px;"
  //var sOptions    = "center:yes; status:no; scroll:no; resizable:no; help:no";
  //var sFeatures   = sDimensions + " " + sOptions;
  
  var sDimensions = "height=290px; width=340px;"
  var sOptions    = "status=no; scrollbars=no; resizable=no";
  var sFeatures   = sDimensions + " " + sOptions;

  //And finally open up the modal dialog
  if (hWin[idxWin] == "")
  {
    hWin[idxWin] = window.open(sURL, "", sFeatures);
    idxWin = 1 - idxWin;
  }
  else
  {
    if (hWin[1 - idxWin] == "")
    {
      hWin[1-idxWin] = window.open(sURL, "", sFeatures);
      idxWin = 1 - idxWin;
    }
    else
    {
...
//    hWin[idxWin] = "";
//  }
//  //hWin[idxWin] = window.showModelessDialog(sURL, "", sFeatures);
//  hWin[idxWin] = window.open(sURL, "", sFeatures);
//  var x = 10;
 }
 catch (error)
 {
   CatchError('LoadPlayerInfo(PlayerID=' + PlayerID + ')', error.number, error.name, error.message);
 }
}
...
function PaintFixtureTeamName(ClassName, TeamName, WidthFlag)
{
 try
 {
  if (WidthFlag == "true")
  {
  	return "<TD class=" + ClassName + " WIDTH=8%>" + TeamName + "</TD>";
  }
  else
  {
...
function PaintFixtureListRow(ClassName, TeamCodeHome, TeamCodeAway, TeamCodeReferee, TheDate, TheTime, Division, TeamNameHome, TeamNameAway, TeamNameReferee, FixtureCode, HomeScore, AwayScore, YYYYMMDD, Venue)
{
 try
 {
  var s = "";
  ...
  s += " <TD class=" + ClassName + " style='text-align:left'>" + Venue + "</TD>";
...
function PaintSquadListRow(ClubCode, ClassName, PlayerName, ClubName, SquadNumber, TopFour)
{
 ...
 {
   CatchError('PaintSquadListRow(...)', error.number, error.name, error.message);
 }  
}
...
function PaintScorecardRow2(TeamNameHome, TeamNameAway, HomeScore, AwayScore, HomeScoreHT, AwayScoreHT)
{
 try
 {
  ...
  s += " <TD class=Ink COLSPAN=2 style='TEXT-ALIGN:center'>";  
  if (HomeScoreHT != null)
  {
    s += " <span style='font-size:8pt;font-weight:normal'>(" + HomeScoreHT + ")&nbsp;&nbsp;</span>";
  }
  s += "v";
  if (AwayScoreHT != null)
  {
    s += "<span style='font-size:8pt;font-weight:normal'>&nbsp;&nbsp;(" + AwayScoreHT + ")</span>";
  }
...
function MouseClick(btnName)
{
 try
 {
  ...
  {
    obj = document.getElementById(CurrentButton);
...
function MouseOver(btnName)
{
 ...
 {
  var obj;
  obj = document.getElementById(btnName);
