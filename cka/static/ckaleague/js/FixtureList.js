...
function PaintFixtureListHeader()
{
  ...
  s += " <TD class=BlackOnOrange WIDTH=18%>Venue</TD>";
  s += " <TD id='ScoreTD' class=BlackOnOrange WIDTH=10%>Score</TD>";
  s += " <TD class=BlackOnOrange WIDTH=6%>Notes</TD>";
  s += "</TR>";
  
  return s;
}

function PaintFixtureListRow(ClassName, TeamCodeHome, TeamCodeAway, TeamCodeReferee, TheDate, TheTime, Division, TeamNameHome, TeamNameAway, TeamNameReferee, FixtureCode, HomeScore, AwayScore, YYYYMMDD, Venue, Period, Month,ChangeStatus,ChangeNotes)
{
  var s = "";
  ...
  // Add change notes if any
  if( ChangeStatus == "") 
  {
    s += "<TD ID='" + idTR + "C9' class=" + ClassName + " style='text-align:left'></TD>";
  }
  else
  {
    var changeHeader="This fixture has moved from when it was originally scheduled. The change has been approved and the reason for this change was:\\n\\n";
    if(ChangeStatus == "CP") {
      changeHeader = "This fixture is pending a move to some other time. The reason for the move is:\\n\\n";
    }
    s += "<TD ID='" + idTR + "C9' class=" + ClassName + ">";
    s += "  <SPAN";
    s += "   OnMouseOver = \"this.style.cursor='pointer'\"";
    s += "   OnMouseOut = \"this.style.cursor='auto'\"";
    s += "   OnClick=\"alert('" + changeHeader + " " + ChangeNotes+ "')\"";
    s += "   style=\"text-decoration:underline\">";
    s += ChangeStatus;
    s += "  </SPAN>";
    s += " </TD>";
