...
function dhtmlXCombo(parent,name,width,optionType,tabIndex){
		...
		if (!window.dhx_glbSelectAr){
			 ...
			 dhtmlxEvent(document.body,"keydown",function(e){ try { if ((e||event).keyCode==9)  window.dhx_glbSelectAr[0].closeAll(); } catch(e) {} if ((e||event).keyCode!=13) return true; } );
		}
...
//<E4GMOD> Copied from a later version of dhtmlxCombo.js
/**
*     @desc: set text in covmbobox
*     @param: text - (string) new text label
*     @type: public
...
*     @topic: 0
*/
		dhtmlXCombo.prototype._createSelf = function(selParent, name, width, tabIndex)
		{
			width=width||100;
            ...
            opt.DrawHeader(this,name, width,tabIndex);
...
dhtmlXCombo_defaultOption.prototype.DrawHeader = function(self, name, width, tabIndex)
{
    var z=document.createElement("DIV");
    z.className = 'dhx_combo_box';
    z._self = self;
	self.DOMelem = z;
    this._DrawHeaderInput(self, name, width, tabIndex);
	this._DrawHeaderButton(self, name, width);
    self.DOMParent.appendChild(self.DOMelem);
}

dhtmlXCombo_defaultOption.prototype._DrawHeaderInput = function(self, name, width, tabIndex)
{
