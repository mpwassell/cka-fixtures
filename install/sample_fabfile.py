...
env.user = "django"
env.git_path = "gitolite@git.versio4.com:v4_website"
...
if not env.hosts: env.hosts = ['hosting.versio4.com']
...
def production():
    env.production = True
    env.appname = "v4_website"
    env.webname = "www.versio4.com versio4.com v4_website.demo.versio4.com"
    env.apppath = "/projects/"+env.appname
    env.activate = 'source %s/bin/activate' % env.apppath
    # if you need to add entries to the user's crontab
    env.cron_entries = [
        "*/15 * * * * cd %s && ./manage.py mycommand" % env.apppath,
    ]
...
def staging():
    ...
    env.appname = "v4_website_test"
    env.webname  = "v4_website_test.demo.versio4.com"
    env.apppath = "/projects/"+env.appname
    env.activate = 'source %s/bin/activate' % env.apppath
